...
function setVportalUrl() {
  ...
  switch (url) {
    case "staging":
      return "https://staging-bvdk.vportal-online.de";
    case "bvdk":
...
async function setStageLogin(competitionId, token) {
  try {
    ...
    if (req?.ok) {
      ...
      await dbUpdate(db, "vportalToken", {
        defaultStage: res?.data?.competitionStageList?.competitionStages[0]?.id,
      });
...
async function getActiveAthlets(
  competitionId,
  competitionStageId,
  competitionGroupId,
  token
